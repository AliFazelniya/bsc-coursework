...
query = """
SELECT p.player_photo_url as _, (p.first_name + ' ' + p.last_name) as Name , p.age as Age, p.nationality as Nationaliy, p.position as Position, p.market_value as Value,  p.OVR , p.goals_number as Goals, p.asists_numbers as Asists, t.team_name as Team
FROM Player as p
...
class LeagueTableWindow(QMainWindow):
    def __init__(self, df):
        super().__init__()
        self.setWindowTitle("League Players Table")
        ...
        self.table_widget.setStyleSheet("QTableWidget { font-size: 20px; }")
